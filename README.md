...
| 模块 | 说明 | 状态 |
|---|---|---|
| **仪表盘** | 概览指标 + **视频本地化流水线工作台**（提取 → 翻译 → 配音 → 替换音轨 → 导出），最近活动、项目列表 | ✅ 已完成 |
| **项目管理** | 多项目归档、状态跟踪（SQLite 持久化） | ✅ 已完成 |
...
| **AI 配音** | 文本 → 语音，情绪 + 语速/音调/音量参数，声音卡片网格，预览播放器 | ✅ Mock 引擎（真实 TTS 待接入） |
| **多人批量配音** | 多说话人分角色批量合成「角色:文本」 | ✅ Mock 引擎 |
| **声音库** | 音色元数据管理（预置音色 + 克隆音色卡、分类筛选） | ✅ Mock 引擎 |
| **声音克隆** | 上传人声片段 → 波形可视化选段 → 克隆；内置合规授权提醒（呼应《深度合成管理规定》） | ✅ Mock 引擎 |
| **视频评论** | 抖音链接 → 分析视频文案 → 生成**字数可控**的评论（已接真实大模型） | ✅ 已接 LLM（DeepSeek 等） |
| **任务队列** | 批量任务异步排队 + 进度持久化（流水线任务实时落库） | ✅ 已完成 |
| **设置** | 偏好与密钥持久化（SQLite） | ✅ 已完成 |
...
仪表盘内置「视频本地化流水线工作台」，一站式把原视频改写成目标语言版本：

```
提取字幕  →  翻译改写  →  AI 配音  →  替换音轨  →  导出
(ffmpeg)     (真实LLM)    (Mock)     (ffmpeg)     (.mp4)
```

- **提取字幕**：复用 `/api/extract` 的 `extractor` 模块，从本地视频或 9 大平台链接抽取内嵌字幕（已修复 `und` 语言字幕无法提取的问题）。
- **翻译改写**：调用真实大模型（豆包 / DeepSeek / 通义 / OpenAI，走 OpenAI 兼容协议）。在 `.env` 放入任一 key 即自动接入；无 key 时降级为占位译文，流水线不中断。
- **AI 配音**：按字幕逐条生成音频（当前为 Mock 占位音轨，时长按字数估算），预留真实 TTS 引擎接口。
- **替换音轨**：用 ffmpeg 把逐条配音按时间轴拼成一条带静音间隔的音轨，再与原画面混流，替换原音轨。
- **导出**：产出带新配音的 `.mp4`，可直接在浏览器预览 / 下载；同时生成「原文 / 译文」字幕对照表。

后端接口：`POST /api/pipeline/run`（上传或链接 + 目标语言 + 音色 → 返回 `job_id`），前端轮询 `GET /api/pipeline/status/{job_id}` 获取五步进度。任务实时落库 `tasks` 表。

> 配音为 Mock 阶段：接入火山 / 讯飞 / ElevenLabs / CosyVoice 等真实引擎后，仅需替换 `src/tts.py` 的 `generate_mock_audio` 实现，接口与返回结构不变。
...
- [x] 视频评论模块（真实 LLM 接入）
- [x] 阶段 1：字幕提取 / 转写 / OCR / 录音 迁入工作台真实页面
- [x] 阶段 2：SQLite 数据层 + 任务队列异步化 + 项目管理 / 设置页
- [x] 阶段 3：AI 配音 / 批量配音 / 声音库 / 声音克隆（Mock 引擎跑通 UI，真实 TTS 接口预留）
- [x] 阶段 4：仪表盘「视频本地化流水线」编排跑通（提取 → 翻译改写 → 配音 → 替换音轨 → 导出）
- [ ] 接入真实 TTS / 声音克隆引擎（火山 / 讯飞 / ElevenLabs / CosyVoice）
- [ ] 抖音真实链接解析打通（视频评论的分析层）
